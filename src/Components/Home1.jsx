...
const url = "https://capstoneproject-goldratecalculator.onrender.com";
...
export default function Home() {
  ...
  const [perGramPrice, setPerGramPrice] = useState(0);
  ...
  const handleCalculate = async () => {
    ...
    if (unit === 'gram') {
      if (carrat === '24k') {
        ...
        setPerGramPrice(price)
      
      } else if (carrat === '22k') {
        calculatedAmount = quantity * (price - 443.15);

        setPerGramPrice(price - 443.15)

      } else if (carrat === '18k') {
        calculatedAmount = quantity * (price - 886.3);
        setPerGramPrice(price - 886.3)
            ...
            {unit === 'gram' ? (
              <table className="table table-hover text-center odd">
                ...
                <tbody>
                  <tr>
                    <td>1 Gram</td>
                    <td>{perGramPrice.toFixed(0)}</td>
                  </tr>
                  <tr>
                    <td>8 Gram</td>
                    <td>{perGramPrice.toFixed(0) * 8}</td>
                    </tr>
                  <tr>
                    <td>10 Gram</td>
                    <td>{perGramPrice.toFixed(0) * 10}</td>
                  </tr>
                  <tr>
                    <td>100 Gram</td>
                    <td>{perGramPrice.toFixed(0) * 100}</td>
                  </tr>
                  <tr>
                    <td><mark>{quantity} Gram</mark></td>
                    <td><mark>{perGramPrice.toFixed(0) * quantity}</mark></td>
                  </tr>
                </tbody>
              </table>
            ) : unit === 'pavan' ? (
